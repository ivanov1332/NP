    ...
    public static void main(String[] args) {
        ...
        if (testCase == 1) { // students
            ...
            if (studentScenario == 1) {
                ...
                try {
                    finalStudents = FilterAndSort.execute(students, student -> student.labAssistantPoints() >= 8.0 && student.getYear() >= 3);
                ...
                try {
                    finalStudents = FilterAndSort.execute(students, student -> (student.grades.size() > student.totalCourses() * 0.9) && student.average() >= 9.0);
                } catch (EmptyResultException e) {
                    System.out.println(e.getMessage());
                }
            ...
            List<Integer> finalIntegers = null;
            try {
                finalIntegers = FilterAndSort.execute(integers, number -> number % 2 == 0 && number % 15 == 0);
                finalIntegers.forEach(System.out::println);
            } catch (EmptyResultException e) {
                System.out.println(e.getMessage());
            }
